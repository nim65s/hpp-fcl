...
class MemberDef(Reference):
    def __init__(self, index, memberdefxml, parent):
        ...
        self.params = tuple([(param.find('type'), param.find('declname'), param.find('array'))
    ...
    def s_args(self):
        ...
        if len(self.parent.template_params) > 0:
            ...
            args = ", ".join([
                self.xmlToType(type, array, parentClass=self.parent, tplargs=tplargs)
    ...
    def s_docstring(self):
        return self.index.xml_docstring.getDocString(self.xml.find('briefdescription'),
                                                     self.xml.find('detaileddescription'), self.index.output)
...
class NamespaceCompound(CompoundBase):
    ...
    def parseEnumSection(self, section):
        for member in section.iterchildren("memberdef"):
            ref = Reference(index=self.index, id=member.attrib["id"], name=self.name + "::" + member.find("name").text)
...
class ClassCompound(CompoundBase):
    ...
    def _writeClassDoc(self, output):
        docstring = self.index.xml_docstring.getDocString(self.definition.find('briefdescription'),
                                                          self.definition.find('detaileddescription'),
                                                          self.index.output)
        attribute_docstrings = ""
        for member in self.attributes:
            _dc = self.index.xml_docstring.getDocString(member.find('briefdescription'),
                                                        member.find('detaileddescription'), self.index.output)
    ...
    def write(self, output):
        ...
        if self.template_specialization:
            output.warn(
                "Disable class {} because template argument are not resolved for templated class specialization.".
                format(self.name))
            return
            ...
            if member.s_name()[0] == '~':
                ...
                output.out(
                    template_constructor_doc.format(
                        tplargs=", ".join(
                            [d['type'] + " " + d['name'] for d in self.template_params + member.template_params]),
        ...
        for prototype, members in member_funcs.items():
            ...
            output.out(
                template_member_func_doc.format(template="template <{}>\n".format(tplargs) if len(tplargs) > 0 else "",
                                                rettype=member.s_rettype(),
                                                classname_prefix=classname_prefix,
                                                argsstring=member.s_prototypeArgs(),
                                                body=body))
...
class Index:
    ...
    def write(self):
        ...
        self.output.out("".join([template_include_intern.format(filename=filename) for filename in includes]))
        ...
        for prototype in prototypes:
            ...
            tplargs = ", ".join(
                [d['type'] + " " + d['name'] for d in member.parent.template_params + member.template_params])
            self.output.out(
                template_static_func_doc.format(template="template <{}>\n".format(tplargs) if len(tplargs) > 0 else "",
                                                rettype=member.s_rettype(),
                                                argsstring=member.s_prototypeArgs(),
                                                body=body))
    ...
    def registerReference(self, obj, overwrite=True):
        if obj.id in self.references:
            if obj.name != self.references[obj.id].name:
                self.output.warn("!!!! Compounds " + obj.id + " already exists.", obj.name,
...
class OutputStreams(object):
    ...
    def writeFooterAndCloseFiles(self):
        for n, f in self._created_files.items():
            # Footer
            self._out = f
            self.out(template_file_footer.format(header_guard=makeHeaderGuard(n), ))
            f.close()
...
if __name__ == "__main__":
    ...
    index = Index(input=sys.argv[1], output=OutputStreams(args.output_directory, sys.stdout, sys.stderr))
    index.parseCompound()
    index.write()
